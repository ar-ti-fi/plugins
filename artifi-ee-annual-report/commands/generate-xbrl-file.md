...
```
/artifi-ee:generate-xbrl-file
```

## Prerequisites

- Annual report data must be prepared (run `/artifi-ee:prepare-annual-report` first or have financial data available)
...
SCRIPT_DIR="$(dirname "$(which claude)" 2>/dev/null || echo "$HOME/.claude")/plugins/artifi-ee-annual-report/scripts"

# If the above doesn't work, use the absolute path from the plugin installation
# The script is at: plugins/artifi-ee-annual-report/scripts/generate_xbrl.py
...
```bash
find ~ -name "generate_xbrl.py" -path "*/artifi-ee-annual-report/*" 2>/dev/null
```
